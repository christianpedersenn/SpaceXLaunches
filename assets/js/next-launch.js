fetch('https://api.spacexdata.com/v4/launches/next').then(response => {
  ...
  document.getElementById('next-launch').insertAdjacentHTML('beforeend', '<div class="card"><div style="overflow: left;"><h1>' + data.name + ' - FLIGHT #' + data.flight_number + '</h1><p id="details">' + data.details + '</p><br><div class="card-container"><div class="left"><p class="">LAUNCH STATUS: <span id="status-header' + data.flight_number + '"></span></p><p>LAUNCH DATE: <span>' + convertedDate + '</span></p></div><div class="right"><br></div></div></div></div>');
